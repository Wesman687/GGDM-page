...
export default function DiscordAuth() {
  const { isAuthenticated, isGGMember, user, loading, error } = useAuth()
      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="text-sm">
          <div className="font-medium text-gray-900">
            {user?.username}
            {user?.discordriminator && user.discordriminator !== '0' && (
              <span className="text-gray-500">#{user.discordriminator}</span>
            )}
          </div>
          <div className="flex items-center space-x-1">
            <span className={`text-xs ${isGGMember ? 'text-green-600' : 'text-red-600'}`}>
              {isGGMember ? '✅ GG Member' : '❌ Not GG Member'}
            </span>
            {error && (
              <span className="text-xs text-orange-600" title={error}>
                ⚠️
              </span>
            )}
          </div>
