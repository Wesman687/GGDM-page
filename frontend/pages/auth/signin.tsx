...
export default function SignIn({ providers: serverProviders }: SignInProps) {
  const [providers, setProviders] = useState(serverProviders)
  const [loading, setLoading] = useState(false)

  // Try to load providers client-side if server-side failed
  useEffect(() => {
    if (!providers || Object.keys(providers).length === 0) {
      setLoading(true)
      getProviders().then((clientProviders) => {
        console.log('Client-side providers:', clientProviders)
        setProviders(clientProviders || {})
        setLoading(false)
      }).catch((error) => {
        console.error('Client-side provider error:', error)
        setLoading(false)
      })
    }
  }, [providers])
          ...
          <div className="max-w-md w-full space-y-8">
            <div className="bg-white py-8 px-6 shadow rounded-lg text-center">
              ...
              <p className="text-gray-600 mb-4">
                Discord authentication is not properly configured. Please check the environment variables.
              </p>
              ...
              <div className="mt-4">
                <button
                  onClick={() => signIn('discord', { callbackUrl: '/suggest' })}
